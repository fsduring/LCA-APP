...
import { MATERIALE_FAKTORER } from '../data/options';
...
export default function MaterialerPage() {
  const { materialer, addMateriale, deleteMaterialEntry, getFaktor } = useData();
  const availableTypes = MATERIALE_FAKTORER;
  const [form, setForm] = useState<MaterialeFormState>({
    dato: today,
    materiale: availableTypes[0]?.type ?? '',
    maengde: '',
    produktNote: '',
    leverandoer: '',
    transportmetodeAbc: '',
  ...
  const faktor = useMemo(() => {
    if (!form.materiale) return null;
    const local = availableTypes.find((item) => item.type === form.materiale);
    if (local) {
      return {
        enhed: local.enhed,
    ...
    return getFaktor(form.materiale);
  }, [availableTypes, form.materiale, getFaktor]);
          ...
          {materialer.map((post) => (
            <article key={post.id} className="data-card">
              ...
              <button
                ...
                onClick={() => deleteMaterialEntry(post.id)}
              >
                Slet
              </button>
            </article>
