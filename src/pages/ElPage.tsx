...
import { EL_FAKTORER } from '../data/options';
...
export default function ElPage() {
  const { el, addEl, deleteElectricityEntry, getFaktor } = useData();
  const availableTypes = EL_FAKTORER;
  const [form, setForm] = useState<ElFormState>({
    dato: today,
  ...
  return (
    <div>
      <section className="card">
        <h2 className="section-title">Registrér elforbrug</h2>
          ...
          {el.map((post) => (
            <article key={post.id} className="data-card">
              ...
              <button
                ...
                onClick={() => deleteElectricityEntry(post.id)}
              >
                Slet
              </button>
            </article>
          ))}
          {el.length === 0 && <p>Ingen registreringer endnu.</p>}
        </div>
      </section>
    </div>
  );
