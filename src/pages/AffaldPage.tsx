...
import { AFFALD_FAKTORER } from '../data/options';
...
export default function AffaldPage() {
  const { affald, addAffald, deleteWasteEntry, getFaktor } = useData();
  const availableTypes = AFFALD_FAKTORER;
  const [form, setForm] = useState<AffaldFormState>({
    dato: today,
    fraktion: availableTypes[0]?.type ?? '',
    maengde: '',
    modtager: '',
  ...
  const faktor = useMemo(() => {
    if (!form.fraktion) return null;
    const local = availableTypes.find((item) => item.type === form.fraktion);
    if (local) {
      return {
        enhed: local.enhed,
    ...
    return getFaktor(form.fraktion);
  }, [availableTypes, form.fraktion, getFaktor]);
          ...
          {affald.map((post) => (
            <article key={post.id} className="data-card">
              ...
              <button
                ...
                onClick={() => deleteWasteEntry(post.id)}
              >
                Slet
              </button>
            </article>
