...
import { VAND_FAKTORER } from '../data/options';
...
export default function VandPage() {
  const { vand, addVand, deleteWaterEntry, getFaktor } = useData();
  const availableTypes = VAND_FAKTORER;
  const [form, setForm] = useState<VandFormState>({
    dato: today,
  ...
  const faktor = useMemo(() => {
    ...
    const local = availableTypes.find((item) => item.type === form.type);
    if (local) {
      return {
        enhed: local.enhed,
        co2FaktorKgPerEnhed: local.faktorKgCo2PerEnhed,
      };
    }
    return getFaktor(form.type);
  }, [availableTypes, form.type, getFaktor]);
  ...
  return (
    <div>
      <section className="card">
        <h2 className="section-title">Registrér vandforbrug</h2>
        <form className="form-grid" onSubmit={handleSubmit}>
          ...
          <label>
            Enhed
            <input type="text" value={faktor?.enhed ?? ''} readOnly />
          </label>
          <label>
            Mængde
            <input
          ...
          {vand.map((post) => (
            <article key={post.id} className="data-card">
              ...
              <button
                ...
                onClick={() => deleteWaterEntry(post.id)}
              >
                Slet
              </button>
            </article>
          ))}
          {vand.length === 0 && <p>Ingen registreringer endnu.</p>}
        </div>
      </section>
    </div>
  );
