...
import { BRAENDSTOF_FAKTORER } from '../data/options';
...
export default function BraendstofPage() {
  const { braendstof, addBraendstof, deleteFuelEntry, getFaktor } = useData();
  const availableTypes = BRAENDSTOF_FAKTORER;
          ...
          {braendstof.map((post) => (
            <article key={post.id} className="data-card">
              ...
              <button
                ...
                onClick={() => deleteFuelEntry(post.id)}
              >
                Slet
              </button>
            </article>
